...
import { ArrowRight, ArrowLeft, AlertTriangle, Thermometer, FlaskConical, Settings, Leaf, CheckCircle, Info } from 'lucide-react'
import { industries, SubProcess } from '../data/industries'
import { IndustryIcon } from '../data/industryIcons'

const SubProcessBlock = ({ sub }: { sub: SubProcess }) => (
  <div className="border border-white/5 rounded-2xl overflow-hidden">
    <div className="bg-white/[0.03] px-6 sm:px-8 py-5 border-b border-white/5">
    ...
    <div className="p-6 sm:p-8 space-y-8">
      ...
      <div>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h4 className="text-sm font-semibold text-amber-400 uppercase tracking-wider">Common Foam Challenges</h4>
        </div>
      ...
      <div>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h4 className="text-sm font-semibold text-brand-400 uppercase tracking-wider">Operating Conditions</h4>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="p-3 rounded-lg bg-white/[0.03] border border-white/5">
              <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">{cond.label}</div>
      ...
      <div>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h4 className="text-sm font-semibold text-brand-400 uppercase tracking-wider">Engineering Approach</h4>
        </div>
        <p className="text-gray-400 text-sm leading-relaxed mb-4">{sub.engineeringApproach.description}</p>
      ...
      <div>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h4 className="text-sm font-semibold text-brand-400 uppercase tracking-wider">Solution Configurations</h4>
        </div>
        <div className="space-y-2">
          {sub.solutionConfigurations.map((config, i) => (
            <div key={i} className="flex items-start space-x-3 p-3 rounded-lg bg-brand-900/10 border border-brand-500/10">
    ...
    <div className="min-h-screen bg-navy-900 pt-20">
      ...
      <section className="section-padding pb-10">
        <div className="container-max">
          <Link to="/solutions" className="text-brand-400 hover:text-brand-300 text-sm font-medium inline-flex items-center mb-6 transition-colors duration-200">
            <ArrowLeft className="w-4 h-4 mr-1" /> All Industries
          </Link>
          <IndustryIcon slug={industry.slug} className="w-12 h-12 text-brand-400 mb-4" />
          <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4">{industry.name}</h1>
          <p className="text-lg text-gray-300 max-w-3xl leading-relaxed">{industry.shortDesc}</p>
        </div>
      </section>
        ...
        <section className="px-4 sm:px-6 lg:px-8 pb-8">
          <div className="container-max">
            <div className="flex items-start space-x-3 p-5 rounded-xl bg-brand-900/15 border border-brand-500/15">
              ...
              <p className="text-gray-300 text-sm leading-relaxed">{industry.positioningNote}</p>
            </div>
          </div>
        </section>
      )}
      ...
      <section className="section-padding bg-navy-950/50">
        <div className="container-max">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white">Sustainability Note</h2>
          </div>
          <div className="p-6 rounded-xl bg-green-900/10 border border-green-500/15">
            <p className="text-gray-300 leading-relaxed">{industry.sustainabilityNote}</p>
          </div>
        </div>
      </section>
        ...
        <div className="container-max">
          <div className="bg-gradient-to-r from-brand-900/20 to-brand-800/10 border border-brand-500/15 rounded-2xl p-10 sm:p-14 text-center">
            ...
            <p className="text-gray-300 mb-6 max-w-2xl mx-auto">
              Tell us about your foam challenge and operating conditions. We'll recommend a solution configured for your specific process.
            </p>
